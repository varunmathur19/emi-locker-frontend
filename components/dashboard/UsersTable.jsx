...
export default function UsersTable({
  ...
  const getRoleSlug = (roleId) => {
    return roleSlugMap[Number(roleId)] || "";
  };

  const selectedRoleId = Number(selectedRole);
  const selectedRoleName = getUserRoleName(selectedRoleId);
  const selectedRoleSlug = getRoleSlug(selectedRoleId);
  ...
  const selectedCountryCode = selectedStateParts[0] || "";
  const selectedStateCode = selectedStateParts[1] || "";
  ...
  const selectedStateName =
    selectedState?.name?.trim().toLowerCase() || "";
  ...
  const handleStatusToggle = async (user) => {
    try {
      ...
      const currentStatus = Number(user?.userStatus ?? 1);
  ...
  const handlePreviousPage = async () => {
    ...
    try {
      ...
      console.error("Status Pagination Error:", error);
  ...
  const filteredUsers = tableUsers.filter((user) => {
    ...
    const matchesState =
      !selectedStateCode ||
      userState ===
        selectedStateCode.trim().toLowerCase() ||
              ...
              {filteredUsers.length > 0 ? (
                filteredUsers.map((user, index) => {
                  ...
                  const actualRoleName =
                    getUserRoleName(actualRoleId);

                  const actualRoleSlug =
                    getRoleSlug(actualRoleId);
